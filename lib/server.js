...
// Keep-alive ping endpoint - sends "Ex-ping" to owner
app.get('/ping', async (req, res) => {
    ...
    try {
        ...
        if (!bot || !bot.sendMessage) {
            printLog('warning', '⚠️ Bot not ready for ping yet');
        ...
        const ownerNumber = ownerData[0]?.number || ownerData[0];

        if (!ownerNumber) {
            printLog('error', '❌ Owner number not configured in data/owner.json');
            return res.status(500).json({
                status: 'error',
                message: 'Owner number not configured'
            });
        }

        const ownerJid = ownerNumber.replace(/[^0-9]/g, '') + '@s.whatsapp.net';
        printLog('info', `📤 Sending Ex-ping to ${ownerJid}`);
        ...
        printLog('success', '✅ Ex-ping message sent successfully');

        res.json({
            status: 'success',
            message: 'Ping received and owner notified',
            timestamp: new Date().toISOString()
        });
    } catch (error) {
        printLog('error', `❌ Ping error: ${error.message}`);
        console.error('[PING] Full error:', error);
        res.status(500).json({
            status: 'error',
