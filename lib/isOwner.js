...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    console.log(`[AUTH DEBUG] Checking Owner: Sender=${senderId} (${senderIdClean}) vs Owner=${settings.ownerNumber} (${ownerNumberClean})`);

    if (senderIdClean === ownerNumberClean) {
        console.log('[AUTH DEBUG] Match! User is Owner.');
        return true;
    }

    const isSudoUser = await isSudo(senderId);
    if (isSudoUser) {
        console.log('[AUTH DEBUG] Match! User is Sudo.');
        return true;
    }
    console.log('[AUTH DEBUG] User is NOT Owner or Sudo.');
