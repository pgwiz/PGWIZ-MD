...
const store = {
  ...
  async saveSetting(chatId, key, value) {
    ...
    this._settingsCache.set(cacheKey, { value, exp: Date.now() + 60000 });
  ...
  async getSetting(chatId, key) {
    ...
    if (backend === 'memory') {
      ...
      try {
        result = await adapters[backend].getSetting(chatId, key);
      } catch (e) {
        console.error(`[STORE] Failed to get setting ${key}:`, e.message)
      }
    }

    this._settingsCache.set(cacheKey, { value: result, exp: Date.now() + 60000 });
    return result;
  },
  ...
  */
  ...
  async setBotMode(mode) {
    ...
    this._cachedBotMode = mode
    this._cachedBotModeTime = Date.now()
  ...
  async getBotMode() {
    const envMode = process.env.MODE || process.env.WORK_TYPE || process.env.WORKTYPE
    if (envMode) {
      return envMode.toLowerCase().trim()
    }
    if (this._cachedBotMode && (Date.now() - (this._cachedBotModeTime || 0) < 15000)) {
      return this._cachedBotMode
    }
    let _settings = null
    try { _settings = require('../settings') } catch (e) {}
    const defaultMode = (_settings && _settings.commandMode) || 'public'
    if (backend === 'memory') {
      const mode = this.botMode || defaultMode
      this._cachedBotMode = mode
      this._cachedBotModeTime = Date.now()
      return mode
    } else {
  ...
  isGroupDisabled(chatId) {
    if (!chatId) return false
    const rawList = process.env.DISABLED_GROUPS || process.env.DISABLE_GROUPS || ''
    if (!rawList) return false
    const disabledList = rawList.split(',').map(s => s.trim().replace('@g.us', ''))
    const cleanId = String(chatId).replace('@g.us', '')
    return disabledList.includes(cleanId)
  },
...
process.on('unhandledRejection', (reason, promise) => {
  if (reason?.message?.includes('Connection Closed') || reason?.output?.statusCode === 428 || reason?.message?.includes('rate-overlimit')) return;
  console.error('[STORE] Unhandled rejection at:', promise, 'reason:', reason);
})

store.getEnvBackedSetting = async function(envKey, defaultValue = '') {
