...
const { handleSongSelection } = require('../plugins/song');
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Ignore messages older than 60 seconds (prevent processing backlog)
        // Ignore messages older than 60 seconds (prevent processing backlog)
        ...
        // console.log(`[DEBUG] Msg Age: ${messageAge.toFixed(1)}s`); // Debug log
        if (messageAge > 60) return;

        // One-line minimal log to debug timing
        const remoteJid = message.key.remoteJid;
        const pushName = message.pushName || 'Unknown';
        const msgType = Object.keys(message.message || {})[0] || 'unknown';
        console.log(`[MSG] ${new Date().toLocaleTimeString()} | From: ${pushName} (${remoteJid.split('@')[0]}) | Type: ${msgType}`);

        // await printMessage(message, sock); // Verbose box logging disabled
        ...
        // Fire-and-forget antidelete storage to prevent lag
        ...
        if (/^\d+$/.test(userMessage)) {
            // Song Selection
            if (await handleSongSelection(sock, chatId, senderId, userMessage, message)) {
                return;
            }
